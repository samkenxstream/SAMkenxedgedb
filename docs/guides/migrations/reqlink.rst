...
.. code-block:: bash
    ...
    fill_expr> <CharacterClass>{}
    Created ./dbschema/migrations/00004.edgeql, id:
    m16uylijqf55zccep5y243up5usza722766xkjfamilsba2vpnb37a

The migration system doesn't know that we've already assigned
``class`` values to all the ``Character`` objects, so it still asks us
for an expression to be used in case any of the objects need it. We
can use the appropriately typed ``{}`` here. It will satisfy the
migration system and as long as all ``Character`` objects have a
``class`` link the migration will work just fine. However, if any
``Character`` objects are missing their ``class`` link the migration
will fail which should allow us to investigate what's going on.
...
.. code-block:: bash
    ...
    'class' of object type 'default::Character'
      Detail: Failing object id is '9f4ac7a8-ac38-11ec-b076-afefd12d7e66'.
...
.. code-block:: bash
    ...
    $ edgedb migrate
    Applied m16uylijqf55zccep5y243up5usza722766xkjfamilsba2vpnb37a
    (00004.edgeql)
